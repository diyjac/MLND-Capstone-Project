...
class Model:
    def __init__(self, model_path, width=16, height=16, log_metrics=False):
    ...
    def create(self):
        with self.tf_session.as_default():
            with self.tf_graph.as_default():
                ...
                self.kmodel.add(Conv2D(filters=6, kernel_size=(5,5), strides=[1,1], padding='valid', activation='elu', name='conv1'))
                self.kmodel.add(Conv2D(filters=12, kernel_size=(5,5), strides=[1,1], padding='valid', activation='elu', name='conv2'))
                self.kmodel.add(Flatten(name='flat'))
                self.kmodel.add(Dense(units=50, activation='elu', name='dense1'))
                self.kmodel.add(Dense(units=25, activation='elu', name='dense2'))
